...
export default class PvTableItem extends Vue {
  @Prop({ default: {}, required: true }) option!: any;
  @Prop({ default: {}, required: true }) item!: any;
  @Model('change') datas!: any;
  @Emit('submit')
  onSubmit(item: object, row: object) {
    return {
      item,
      row
    };
  ...
  setPopoverIsShow(row: object, item: any, text: boolean) {
  ...
  setLableText(row: object, item: any) {
    const conetent = findObjectValue(row, item.request || item.name);
